...
'''
    A program that implements Neville and Spline Cubic interpolation methods.
'''
...
def createTable():
    ...
    for i in range(lenOfTable):
        print(f'enter a value for point {i+1} : \n')
        x = float(input("x :\n"))
        datax.append(x)
        y = float(input("y :\n"))
        datay.append(y)
...
def Startspline(listx, listy, findpoint):
    ...
    print(f'\nf({findpoint}) = {cubic_spline(listx, listy, findpoint, h, M, i)}\n')
...
print("\nNEVILLE METHOD : \n")
print(f'\nf({findpoint}) = {neville(listx, listy, findpoint)}')
